...
static mim_conn_handle_t _mim_menu_conn = NULL;
...
static void _param_info_ip_address(crsf_device_param_read_value_t *value) {
    ip4_addr_t addr = mim_conn_get_mim_addr(_mim_menu_conn);
...
static void _param_info_skymap_ip_address(crsf_device_param_read_value_t *value) {
    ip4_addr_t addr = mim_conn_get_skymap_addr(_mim_menu_conn);
    snprintf(_mim_menu_ip_address_str, 16, IPSTR, IP2STR(&addr));
    value->info = _mim_menu_ip_address_str;
}
...
static void _param_folder_guidance_get(crsf_device_param_read_value_t *value) { value->folder = "guidance"; }
...
static void _param_folder_nav_attack_get(crsf_device_param_read_value_t *value) { value->folder = "attack angle"; }
...
static void _param_folder_nav_pitcher_get(crsf_device_param_read_value_t *value) { value->folder = "pitch control"; }
...
static void _param_command_test_rc_get(crsf_device_param_read_value_t *value) {
    ...
    if (value->command.state == CRSF_COMMAND_STATE_PROGRESS) {
        ...
        snprintf(_mim_menu_command_status,
                 _MIM_MENU_MAX_COMMAND_STATUS_LENGTH,
                 "%s\x1E"
                 "roll %u\x1E"
                 "pitch %u",
                 label,
...
static void _param_command_test_rc_set(const crsf_device_param_write_value_t *value) {
    ...
    case CRSF_COMMAND_STATE_POLL:
        ...
        case _MIM_MENU_TEST_RC_ROLL_RIGHT:
            ...
            break;
        case _MIM_MENU_TEST_RC_PITCH_UP:
            mim_rc_set_override(MIM_RC_CHANNEL_PITCH,
                                pitch_inverted ? CRSF_RC_CHANNELS_MIN : CRSF_RC_CHANNELS_MAX,
                                MIM_RC_OVERRIDE_LEVEL_TEST);
            break;
        case _MIM_MENU_TEST_RC_PITCH_DOWN:
            mim_rc_set_override(MIM_RC_CHANNEL_PITCH,
                                pitch_inverted ? CRSF_RC_CHANNELS_MAX : CRSF_RC_CHANNELS_MIN,
                                MIM_RC_OVERRIDE_LEVEL_TEST);
            break;
        default:
            break;
...
void mim_menu_init(crsf_device_t *device, mim_nav_handle_t nav, mim_conn_handle_t conn) {
    ...
    crsf_device_param_t *config
        = crsf_device_add_param(device, "config", CRSF_PARAM_TYPE_FOLDER, NULL, _param_folder_guidance_get, NULL);

    crsf_device_add_param(
        device, "N factor", CRSF_PARAM_TYPE_FLOAT, config, _param_float_nav_N_get, _param_float_nav_N_set);
    crsf_device_add_param(device,
                          "max roll",
    ...
    crsf_device_param_t *guidance_attack = crsf_device_add_param(
        device, "attack angle", CRSF_PARAM_TYPE_FOLDER, config, _param_folder_nav_attack_get, NULL);
    ...
    crsf_device_add_param(device,
                          "factor",
                          CRSF_PARAM_TYPE_FLOAT,
                          guidance_attack,
                          _param_float_nav_attack_factor_get,
                          _param_float_nav_attack_factor_set);

    crsf_device_param_t *guidance_pitcher
        = crsf_device_add_param(device, "pitch", CRSF_PARAM_TYPE_FOLDER, config, _param_folder_nav_pitcher_get, NULL);
    ...
    crsf_device_add_param(device,
                          "pitch invert",
                          CRSF_PARAM_TYPE_SELECT,
                          guidance_pitcher,
                          _param_select_nav_pitcher_inverted_get,
                          _param_select_nav_pitcher_inverted_set);

    crsf_device_add_param(
        device, "test RC", CRSF_PARAM_TYPE_COMMAND, NULL, _param_command_test_rc_get, _param_command_test_rc_set);

    crsf_device_add_param(device, "mode", CRSF_PARAM_TYPE_SELECT, NULL, _param_select_mode_get, _param_select_mode_set);

    crsf_device_add_param(device, "address", CRSF_PARAM_TYPE_INFO, NULL, _param_info_ip_address, NULL);

    crsf_device_add_param(device, "skymap addr", CRSF_PARAM_TYPE_INFO, NULL, _param_info_skymap_ip_address, NULL);
